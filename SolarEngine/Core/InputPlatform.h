...
namespace Solar
{
    class SOLAR_API KeyInputPlatform
    {
	public:
        virtual ~KeyInputPlatform() = default;

		inline int32_t GetValue(int32_t keyCode) const { return (IsPressed(keyCode) || IsHeld(keyCode)) ? 1 : 0; }
	...
	class SOLAR_API MouseInputPlatform
    {
	public:
		virtual ~MouseInputPlatform() = default;

		inline int8_t GetValue(int32_t button) const { return (IsPressed(button) || IsHeld(button)) ? 1 : 0; }
		...
		inline void SetOffset(float offsetX, float offsetY) { mOffsetX = offsetX; mOffsetY = offsetY; }
		inline void SetOffsetX(float offsetX) { mOffsetX = offsetX; }
		inline void SetOffsetY(float offsetY) { mOffsetY = offsetY; }

		inline void SetScroll(float scrollX, float scrollY) { mScrollX = scrollX; mScrollY = scrollY; }
