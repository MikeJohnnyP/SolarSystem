...
{   
    ...
    #define EVENT_CLASS(type) inline virtual std::string GetName() const override { return #type; } \
    ...
    class SOLAR_API EventDispatcher
    {
    public:
        EventDispatcher(Event& event) : m_event(event) {}

        template<typename T>
        bool Dispatcher(EventCallback<T> callback)
        {
